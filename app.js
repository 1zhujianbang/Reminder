...
(function () {
    ...
    function loadBookmarks() {
        ...
        if (storageAPI) {
            fetch(storageAPI + '/bookmarks').then(function (r) { return r.json(); }).then(function (d) {
                if (d && d.newsflash && d.newsflash.length > 0) {
                    var localIds = nfBookmarks.map(function (b) { return b.link || b.publishTimestamp || ''; });
                ...
                if (d && d.article && d.article.length > 0) {
                    var localIds2 = artBookmarks.map(function (b) { return b.link || b.publishTimestamp || ''; });
    ...
    function saveBookmarks() {
        ...
        if (storageAPI) {
            ...
            }).catch(function (e) { console.log('[存储] 书签写入错误:', e); });
        }
    }
    ...
    function isBookmarked(item, type) {
        ...
        var id = item.link || item.publishTimestamp || '';
        return list.some(function (b) { return (b.link || b.publishTimestamp || '') === id; });
    }

    function toggleBookmark(item, type) {
        var list = type === 'article' ? artBookmarks : nfBookmarks;
        var id = item.link || item.publishTimestamp || '';
        var idx = list.findIndex(function (b) { return (b.link || b.publishTimestamp || '') === id; });
        if (idx >= 0) {
            list.splice(idx, 1);
        } else {
            list.push(item);
    ...
    var DRAW_COLORS = ['#22c55e', '#ef4444', '#1f6feb', '#f59e0b', '#a855f7', '#ec4899'];
    var colorIdx = 0;
        ...
        fetchData: async function (instId, interval, limit) {
            ...
            while (allData.length < limit) {
                ...
                var url = 'https://www.okx.com/api/v5/market/candles?instId=' +
                    encodeURIComponent(instId) + '&bar=' + interval + '&limit=' + fetchLimit;
            ...
            return allData.map(function (c) {
                ...
                return d.time > 0 && isFinite(d.open) && isFinite(d.high) && isFinite(d.low) && isFinite(d.close);
            });
        },
    });
    ...
    EXCHANGES.push({
        ...
        tfMap: { 1: '1m', 5: '5m', 15: '15m', 60: '1h', 240: '4h', 1440: '1d' },
        fetchData: async function (instId, interval, limit) {
            var url = 'https://fapi.binance.com/fapi/v1/klines?symbol=' +
                encodeURIComponent(instId) + '&interval=' + interval + '&limit=' + limit;
            var r = await fetch(url);
            if (!r.ok) throw new Error('HTTP ' + r.status);
            var d = await r.json();
    ...
    EXCHANGES.push({
        ...
        fetchData: async function (coin, interval, limit) {
            ...
            var r = await fetch('https://api.hyperliquid.xyz/info', {
                ...
                body: JSON.stringify({
                    type: 'candleSnapshot',
                    req: { coin: coin, interval: interval, startTime: startTime, endTime: now },
                }),
            });
            if (!r.ok) throw new Error('HTTP ' + r.status);
            var d = await r.json();
    ...
    function loadSignals() {
        ...
        if (storageAPI) {
            fetch(storageAPI + '/signals').then(function (r) { return r.json(); }).then(function (data) {
    ...
    function saveSignals() {
        ...
        if (storageAPI) {
            ...
            }).catch(function (e) { console.log('[存储] 写入错误:', e); });
        }
    }
    ...
    function placeSignal(time, type) {
        ...
        showToast('已标记: ' + cfg.label + ' @ ' + formatPrice(candle ? candle.close : 0), 'success');
    ...
    function toggleFullscreenChart() {
        ...
        setTimeout(function () {
            chart.applyOptions({ width: dom.chartEl.clientWidth, height: dom.chartEl.clientHeight });
    ...
    function loadDrawings() {
        try { drawLines = JSON.parse(localStorage.getItem('trading_drawings')) || {}; }
        catch (_) { drawLines = {}; }
        if (storageAPI) {
            fetch(storageAPI + '/drawings').then(function (r) { return r.json(); }).then(function (d) {
                if (d && Object.keys(d).length > 0) {
                    for (var sym in d) {
                        if (!drawLines[sym]) drawLines[sym] = [];
                        d[sym].forEach(function (line) {
                            if (!drawLines[sym].some(function (l) { return l.price === line.price && l.color === line.color; })) {
    ...
    function saveDrawings() {
        ...
        if (storageAPI) {
            ...
            }).catch(function (e) { console.log('[存储] 绘制线写入错误:', e); });
        }
    }
    ...
    function toggleDrawMode() {
        drawMode = !drawMode;
        document.getElementById('chart-container').classList.toggle('draw-mode', drawMode);
    ...
    function initChart() {
        ...
        chart.subscribeCrosshairMove((param) => {
            if (param.time && param.seriesData && param.seriesData.has(candleSeries)) {
                ...
                if (!pinnedTime) {
                    state.selectedTime = param.time;
                    state.selectedIndex = state.data.findIndex((d) => d.time === param.time);
                    updateNoteArea();
                }
            } else if (!pinnedTime) {
    ...
    function updateOHLCBar(candle) {
        ...
        const fullData = state.data.find(function (cd) { return cd.time === time; });
        ...
        document.title = 'Reminder | ' + state.symbol + ' ' + lastPrice + ' ' + lastDirection;
    }
    ...
    function updateDataSource() {
        ...
        if (state.realtime && realtimeSource) {
            var label = realtimeSource === 'okx-ws' ? 'WS(OKX)' :
                        realtimeSource === 'binance-ws' ? 'WS(Binance)' :
                        'REST';
            parts.push('🟢 ' + label);
        }
    ...
    async function loadChartData() {
        try {
            ...
            if (state.data.length > 0) {
                marketPriceLine.applyOptions({ price: state.data[state.data.length - 1].close });
                var c = state.data[state.data.length - 1];
                document.title = 'Reminder | ' + state.symbol + ' ' + formatPrice(c.close) + ' ' + (c.close > c.open ? '↑' : c.close < c.open ? '↓' : '→');
            }
            chart.timeScale().fitContent();
            renderDrawings(state.symbol);
            updateMarkers();
    ...
    async function fetchNewsflash() {
        ...
        try {
            var url = 'https://api.odaily.news/api/v1/newsflash' +
                (nfCategory ? '/' + nfCategory : '') + '?page=1&size=30&lang=zh-cn';
            var r = await fetch(url);
            if (!r.ok) throw new Error('HTTP ' + r.status);
            var j = await r.json();
    ...
    async function fetchArticles() {
        ...
        articleLoading = true;
        try {
            var url = 'https://api.odaily.news/api/v1/article' +
                (articleCategory ? '/' + articleCategory : '') + '?page=1&size=30&lang=zh-cn';
            var r = await fetch(url);
            if (!r.ok) throw new Error('HTTP ' + r.status);
            var j = await r.json();
            if (j.code === 200 && j.data && j.data.list) {
    ...
    async function searchContent(keyword) {
        ...
        searchLoading = true;
        try {
            var url = 'https://api.odaily.news/api/v1/search/' + searchSource +
                '?keyword=' + encodeURIComponent(keyword) + '&page=1&size=20&lang=zh-cn';
            var r = await fetch(url);
            if (!r.ok) throw new Error('HTTP ' + r.status);
            var j = await r.json();
            if (j.code === 200 && j.data && j.data.list) {
    ...
    function renderNewsflash() {
        ...
        el.innerHTML = data.map(function (item) {
            ...
            return '<div class="news-entry' + impClass + '" data-url="' + escapeHtml(linkUrl) + '">' +
                '<div class="news-top">' +
                '<span class="news-dot"></span>' +
                '<span class="news-time">' + timeStr + '</span>' +
                (item.isImportant ? '<span class="news-badge">重要</span>' : '') +
                '<span class="news-bm" data-bm="newsflash" data-bmid="' + escapeHtml(linkUrl || String(item.publishTimestamp || '')) + '" style="margin-left:auto;cursor:pointer;font-size:13px;">' + (isBm ? '★' : '☆') + '</span>' +
                '</div>' +
                (title ? '<div class="news-title">' + escapeHtml(title) + '</div>' : '') +
                '<div class="news-content">' + escapeHtml(contentPlain.slice(0, 300)) + '</div>' +
                (item.sourceUrl ? '<div class="news-source">↗ ' + escapeHtml(item.sourceUrl) + '</div>' : '') +
                '</div>';
        }).join('');
    }
    ...
    function renderArticles() {
        ...
        el.innerHTML = data.map(function (item) {
            ...
            return '<div class="news-entry" data-url="' + escapeHtml(linkUrl) + '">' +
                ...
                '<span class="news-dot"></span>' +
                '<span class="news-time">' + timeStr + '</span>' +
                '<span class="news-bm" data-bm="article" data-bmid="' + escapeHtml(linkUrl || String(item.publishTimestamp || '')) + '" style="margin-left:auto;cursor:pointer;font-size:13px;">' + (isBm ? '★' : '☆') + '</span>' +
                '</div>' +
                '<div style="display:flex;gap:8px;align-items:flex-start;">' +
                '<img src="' + imgSrc + '" style="width:60px;height:40px;object-fit:cover;border-radius:4px;flex-shrink:0;">' +
                '<div style="flex:1;min-width:0;">' +
                (title ? '<div class="news-title">' + escapeHtml(title) + '</div>' : '') +
                '<div class="news-content">' + escapeHtml(contentPlain.slice(0, 200)) + '</div>' +
                '</div></div>' +
                (item.sourceUrl ? '<div class="news-source">↗ ' + escapeHtml(item.sourceUrl) + '</div>' : '') +
                '</div>';
        }).join('');
    ...
    function generateArticleImage(title) {
        ...
        if (ctx.measureText(text).width > maxW) {
            ...
            for (var i = 0; i < words.length; i++) {
                if (ctx.measureText(line1 + words[i]).width < maxW && line1.length < text.length * 0.6) {
                    line1 += words[i];
                } else {
                    line2 += words[i];
                }
    ...
    function updateCandleFromRealtime(candle) {
        ...
        candleSeries.update({ time: tip.time, open: tip.open, high: tip.high, low: tip.low, close: tip.close });
        volumeSeries.update({
            time: tip.time,
            value: tip.volume,
            color: tip.close >= tip.open ? 'rgba(34,197,94,0.3)' : 'rgba(239,68,68,0.3)',
        });
        marketPriceLine.applyOptions({ price: tip.close });
        document.title = 'Reminder | ' + state.symbol + ' ' + formatPrice(tip.close) + ' ' + (tip.close > tip.open ? '↑' : tip.close < tip.open ? '↓' : '→');
    ...
    function connectOKXWS() {
        ...
        console.log('[OKX WS] connecting to ' + wsUrl + ' channel=' + channel + ' instId=' + instId);
        ...
        ws.onopen = function () {
            ...
            updateDataSource();
            ws.send(JSON.stringify({ op: 'subscribe', args: [{ channel: channel, instId: instId }] }));
        };
        ...
        ws.onmessage = function (ev) {
            try {
                ...
                if (msg.event === 'subscribe') {
                    console.log('[OKX WS] subscribed:', msg.arg && msg.arg.channel, msg.arg && msg.arg.instId);
                    return;
                }
                if (msg.event === 'error') {
                ...
                if (msg.arg && msg.data && msg.data[0]) {
                    ...
                    updateCandleFromRealtime({ time: ts, open: open, high: high, low: low, close: close, volume: volume });
                }
            } catch (e) {
    ...
    var BINANCE_WS_TF = { 1: '1m', 5: '5m', 15: '15m', 60: '1h', 240: '4h', 1440: '1d' };
    ...
    async function restPollUpdate() {
        try {
            ...
            if (oldLast && newLast && oldLast.time === newLast.time &&
                oldLast.close === newLast.close && oldLast.high === newLast.high && oldLast.low === newLast.low) {
                return;
            }
            ...
            try { candleSeries.update({ time: tip.time, open: tip.open, high: tip.high, low: tip.low, close: tip.close }); } catch (_) {}
            try {
                volumeSeries.update({
                    time: tip.time,
                    value: tip.volume,
    ...
    function updateMarkers() {
        ...
        for (const [timeStr, sig] of Object.entries(signals)) {
            ...
            if (!cfg) continue;
            markers.push({
                time, position: cfg.pos, shape: cfg.shape,
                color: cfg.color, text: cfg.label,
            });
            // Note indicator dot
        ...
        if (showStructures && state.data.length > 10) {
            ...
            swings.forEach(function (s) {
                ...
                markers.push({
                    ...
                    color: s.type === 'high' ? 'rgba(239,68,68,0.5)' : 'rgba(34,197,94,0.5)',
    ...
    function initDepthChart() {
        ...
        closeBtn.addEventListener('mouseenter', function () { this.style.background = 'var(--bg)'; });
        closeBtn.addEventListener('mouseleave', function () { this.style.background = 'var(--bg3)'; });
    ...
    function renderDepthChart(asks, bids) {
        ...
        depthAskSeries = depthChartIns.addAreaSeries({
            ...
            topColor: 'rgba(239,68,68,0.3)', bottomColor: 'rgba(239,68,68,0.01)',
        });
        depthBidSeries = depthChartIns.addAreaSeries({
            lineColor: '#22c55e', lineWidth: 1,
            topColor: 'rgba(34,197,94,0.3)', bottomColor: 'rgba(34,197,94,0.01)',
        });
    ...
    async function fetchOrderBook() {
        ...
        try {
            ...
            if (j.code === '0' && j.data && j.data[0]) {
                var asks = j.data[0].asks.map(function (a) { return { price: parseFloat(a[0]), size: parseFloat(a[1]) }; }).filter(function (a) { return a.price > 0 && a.size > 0; });
                var bids = j.data[0].bids.map(function (b) { return { price: parseFloat(b[0]), size: parseFloat(b[1]) }; }).filter(function (b) { return b.price > 0 && b.size > 0; });
    ...
    function processDepth(asks, bids) {
        ...
        for (var i = asks.length - 1; i >= 0; i--) {
            total += asks[i].size;
            askCum.unshift({ price: asks[i].price, total: total, index: askCum.length });
        }
    ...
    async function toggleDepthChart(force) {
        var show = force !== undefined ? force : !depthDiv || depthDiv.style.display === 'none';
        if (show) {
    ...
    function renderToolbar() {
        ...
        // --- Timeframe buttons ---
        dom.timeframeGroup.innerHTML = TIMEFRAMES.map(
            function (tf) {
        ...
        dom.timeframeGroup.addEventListener('click', async function (e) {
            ...
            dom.timeframeGroup.querySelectorAll('.tf-btn').forEach(function (b) { b.classList.remove('active'); });
    ...
    function updateNoteArea() {
        ...
        if (sig) {
            ...
            dom.noteSignalInfo.textContent =
                SIGNAL_CFG[sig.type].label + ' @ ' + formatPrice(sig.price) + ' — ' + formatTime(t);
    ...
    function showNotePicker() {
        ...
        doPicker('note-picker', 'note-picker-list', allNotes, function (item) {
            return {
                url: '',
                title: '[' + item.symbol + '] ' + formatTime(item.time) + ' ' + item.noteText,
                ts: item.time,
    ...
    function doPicker(pickerId, listId, data, mapFn) {
        ...
        list.innerHTML = data.slice(0, 30).map(function (item) {
            ...
            return '<div class="flash-picker-item" ' +
        ...
        list.onclick = function (e) {
            ...
            var prefix = pickerId === 'article-picker' ? '文章' :
                         pickerId === 'note-picker' ? '笔记' : '快讯';
            var ref;
            if (prefix === '笔记' && extra) {
    ...
    function updateSignalControls() {
        ...
        if (noteTab === 'newsflash') {
            var opts = NF_CATEGORIES.map(function (c) {
                return '<option value="' + c.endpoint + '"' + (c.endpoint === nfCategory ? ' selected' : '') + '>' + c.label + '</option>';
            }).join('');
            ctrl.innerHTML =
                '<span style="color:var(--text);opacity:0.6;">分类</span> ' +
                '<select id="nf-cat" class="sig-ctrl-select">' + opts + '</select> ' +
            ...
            var opts2 = ARTICLE_CATEGORIES.map(function (c) {
                return '<option value="' + c.endpoint + '"' + (c.endpoint === articleCategory ? ' selected' : '') + '>' + c.label + '</option>';
            }).join('');
            ctrl.innerHTML =
                '<span style="color:var(--text);opacity:0.6;">分类</span> ' +
                '<select id="art-cat" class="sig-ctrl-select">' + opts2 + '</select> ' +
            ...
            ctrl.innerHTML =
                ...
                '<option value="article"' + (searchSource === 'article' ? ' selected' : '') + '>文章</option>' +
                '</select> ' +
    ...
    function renderSignalLog() {
        ...
        // Filter by type
        if (signalFilter !== 'all') {
            entries = entries.filter(function (e) { return e.type === signalFilter; });
        }
        ...
        if (header) {
            ...
            if (noteTab === 'signal') {
                ...
                header.innerHTML =
                    ...
                    '信号记录 <span class="sig-count">' + allCount + '</span>' +
                    '<span class="sig-stats">' +
                    '<span class="sig-stat-up">▲' + stats.buy + '</span> ' +
                    '<span class="sig-stat-mid">◆' + stats.hold + '</span> ' +
                    '<span class="sig-stat-down">▼' + stats.sell + '</span>' +
                    '</span></span>' +
                    '<span class="sig-filters">' +
                    filters.map(function (f) {
                        return '<button class="sig-filter' + (f.key === signalFilter ? ' active' : '') + '" data-filter="' + f.key + '">' + f.label + '</button>';
                    }).join('') +
                    '</span>';
                ...
                var catFilters = [{ key: 'all', label: '全部' }].concat(NOTE_CATEGORIES.map(function (c) {
                    return { key: c.key, label: c.label };
                }));
                header.innerHTML =
                    tabsHtml +
                    '<span class="sig-header-left">' +
                    '笔记列表 <span class="sig-count">' + noteCount + '</span>' +
                    '</span>' +
                    '<span class="sig-filters">' +
                    catFilters.map(function (f) {
                ...
                header.innerHTML =
                    tabsHtml +
                    '<span class="sig-header-left">' +
                    '快讯 <span class="sig-count">' + nfCount + '</span>' +
                    '</span>' +
                ...
                header.innerHTML =
                    tabsHtml +
                    '<span class="sig-header-left">' +
                    '文章 <span class="sig-count">' + artCount + '</span>' +
                    '</span>' +
        ...
        dom.signalList.innerHTML = entries
            .map(function (entry) {
                ...
                var catBadge = noteData.text ?
                    '<span class="sig-cat" style="background:' + cat.color + '22;color:' + cat.color + '">' + cat.label + '</span>' :
                    '';
                var noteHtml = noteData.text ?
                    '<div class="sig-note">' + catBadge + renderNoteText(noteData.text) + '</div>' :
                    '';
                return '<div class="sig-entry" data-time="' + entry.time + '">' +
                    '<span class="sig-dot" style="background:' + cfg.color + '"></span>' +
                    '<span class="sig-info">' +
                    '<span class="sig-type">' + cfg.label + '</span>' +
                    '<span class="sig-meta">' + formatTime(entry.time) + ' @ ' + formatPrice(entry.price) + '</span>' +
                    '</span>' +
                    noteHtml +
                    '<button class="sig-del" data-time="' + entry.time + '" title="删除信号">×</button>' +
                    '</div>';
            })
            .join('');
        updateSignalControls();
    ...
    function renderNoteList() {
        ...
        el.innerHTML = entries.map(function (e) {
            ...
            return '<div class="note-entry" data-time="' + e.time + '">' +
                ...
                '<div class="note-body">' +
                '<span class="note-meta">' + cat.label + '</span>' +
                '<div class="note-text">' + renderNoteText(e.note.text) + '</div>' +
                '<div class="note-time">' + formatTime(e.time) + '</div>' +
                '</div></div>';
        }).join('');
    }
        ...
        el.addEventListener('click', async function (e) {
            ...
            if (bm) {
                ...
                if (!item && searchResults) {
                    item = searchResults.find(function (c) { return (c.link || String(c.publishTimestamp || '')) === id; });
                }
                if (item) {
                    ...
                    if ((type === 'article' && articleCategory === '__bookmarked__') ||
    ...
    function bindToolActions() {
        ...
        document.getElementById('btn-live').addEventListener('click', toggleRealtime);
    ...
    function renderNoteText(text) {
        ...
        return parts.map(function (part) {
            ...
            if (flashMatch) {
                ...
                return '<a class="note-ref note-ref-flash" href="' + escapeHtml(flashMatch[2]) + '" target="_blank" title="' + title + '">📰 ' + title + '</a>';
            }
            var articleMatch = part.match(/^【文章:([^|]+)\|([^】]+)】$/);
            if (articleMatch) {
                var artTitle = escapeHtml(articleMatch[1]);
                return '<a class="note-ref note-ref-article" href="' + escapeHtml(articleMatch[2]) + '" target="_blank" title="' + artTitle + '">📄 ' + artTitle + '</a>';
            }
    ...
    function initSignalControls() {
        ...
        function doSearch() {
            ...
            if (!kw) return;
            searchSource = document.getElementById('search-source') ? document.getElementById('search-source').value : 'newsflash';
