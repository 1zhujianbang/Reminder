...
// ============================================================
import { NF_CATEGORIES, ARTICLE_CATEGORIES } from './config.js';
import { nfBookmarks, artBookmarks } from './state.js';
...
export async function searchContent(keyword) {
    ...
    try {
        ...
        if (!r.ok) throw new Error('HTTP ' + r.status);
        var j = await r.json();
        if (j.code === 200 && j.data && j.data.list) searchResults = j.data.list;
        else searchResults = [];
